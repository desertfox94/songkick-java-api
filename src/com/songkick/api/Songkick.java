...
public class Songkick {
	...
	public List<Event> getEvents(EventFilter ef) throws IOException {
		...
		if (ef.getType() != null) {
			url = url + "&type=" + ef.getType();
		}

		if (ef.getLocation() != null) {
			if (ef.getLocation().isClientIp()) {
				url = url + "&location=clientip";
			} else if (ef.getLocation().getIp() != null) {
				url = url + "&location=ip:" + ef.getLocation().getIp();
			} else if (ef.getLocation().getMetroId() != null) {
				url = url + "&location=sk:" + ef.getLocation().getMetroId();
			} else if (ef.getLocation().getLat() != Double.MAX_VALUE && ef.getLocation().getLng() != Double.MAX_VALUE) {
				url = url + "&location=geo:" + ef.getLocation().getLat() + "," + ef.getLocation().getLng();
			}
		}
	...
	private List<? extends Object> getSinglePage(String url, Class<? extends ResultsPage> parsingClass, int page) throws IOException {
		...
		if (rp.getResultsPageContents().getResults() == null) {
			return new ArrayList<Object>();
		}
	...
	private List<? extends Object> getAllPages(String url, Class<? extends ResultsPage> parsingClass) throws IOException {
		...
		while (page != null && (page.size() > 0)) {
			...
			if (page.size() == PAGE_SIZE) {
				page = getSinglePage(url, parsingClass, ++pageNum);
			} else {
				page = null;
			}
		}
